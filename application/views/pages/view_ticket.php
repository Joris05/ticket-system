    ...
    <div class="col mb-2">
        <div class="card h-100">
            ...
            <div class="card-footer">
                <small class="text-muted"><?= date('F d, Y h:ia', strtotime($ticket->date_created)); ?></small>
            </div>
        </div>
    </div>
    <button type="button"  onclick="window.location='<?= base_url()?>ticket/status/<?= $ticket->id; ?>/open'" class="btn btn-success">Open</button>
    <button type="button" onclick="window.location='<?= base_url()?>ticket/status/<?= $ticket->id; ?>/close'" class="btn btn-danger">Partially Close</button>
</div>